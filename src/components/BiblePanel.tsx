...
import BibleConfigModal, { BibleLayoutId } from './BibleConfigModal';
...
// Bible Layout Template System — moved to BibleConfigModal.tsx
...
function BiblePanel({
  ...
  const biblePaginationEnabled = bs.biblePaginationEnabled;
  ...
  // Virtualized scroll state (must be after versesList)
  const verseListRef = React.useRef<HTMLDivElement | null>(null);
  const [verseScrollTop, setVerseScrollTop] = React.useState(0);
  const ITEM_ESTIMATED_HEIGHT = 72;
  const VERSES_BUFFER = 5;
  const visibleVerses = useMemo(() => {
    const container = verseListRef.current;
    if (!container) return { start: 0, end: versesList.length };
    const containerHeight = container.clientHeight;
    const totalHeight = versesList.length * ITEM_ESTIMATED_HEIGHT;
    const start = Math.max(0, Math.floor(verseScrollTop / ITEM_ESTIMATED_HEIGHT) - VERSES_BUFFER);
    const end = Math.min(versesList.length, Math.ceil((verseScrollTop + containerHeight) / ITEM_ESTIMATED_HEIGHT) + VERSES_BUFFER);
    return { start, end, totalHeight };
  }, [versesList.length, verseScrollTop]);
  ...
  // Synchronize active list item viewport position (virtualized)
  useEffect(() => {
    if (verseListRef.current) {
      const targetTop = (activeVerse - 1) * ITEM_ESTIMATED_HEIGHT - 20;
      const currentTop = verseListRef.current.scrollTop;
      const viewBottom = currentTop + verseListRef.current.clientHeight;
      if (targetTop < currentTop || targetTop + ITEM_ESTIMATED_HEIGHT > viewBottom) {
        verseListRef.current.scrollTop = targetTop;
      }
    }
  }, [activeVerse]);
  ...
  const handleXMLUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const parsedVerses = await new Promise<{ book: string; chapter: number; verse: number; text: string; bookNumber?: number }[]>((resolve, reject) => {
          const worker = new Worker(new URL('../workers/bibleParser.worker.ts', import.meta.url), { type: 'module' });
          worker.onmessage = (e) => {
            if (e.data.type === 'progress') {
              setXmlProgress(e.data.progress);
            } else if (e.data.type === 'result') {
              worker.terminate();
              resolve(e.data.verses);
          ...
          worker.postMessage({ type: 'parse', xmlString: textStr });
        });
        console.log(`Bible import: parsed ${parsedVerses.length} verses from XML`);
  ...
  return (
    <div className="flex-1 flex min-h-0 bg-neutral-900 h-full select-none" id="bible-grid-dashboard">
      ...
      <div className="w-[320px] bg-neutral-950 border-r border-zinc-800 flex flex-col h-full shrink-0" id="verse-list-sidebar">
        ...
        {/* Verse Items List Body [Virtualized Scrollable] */}
        <div
          ref={verseListRef}
          className="flex-1 overflow-y-auto custom-scrollbar bg-neutral-950/60"
          onScroll={() => {
            if (verseListRef.current) {
              setVerseScrollTop(verseListRef.current.scrollTop);
            }
          }}
        >
          <div style={{ height: versesList.length * ITEM_ESTIMATED_HEIGHT, position: 'relative' }}>
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '10px' }}>
              <div className="space-y-2" style={{ marginTop: visibleVerses.start * ITEM_ESTIMATED_HEIGHT }}>
                {versesList.slice(visibleVerses.start, visibleVerses.end).map((verse) => (
                  <VerseItem
                    key={verse.num}
                    num={verse.num}
      ...
      <div className="flex-1 flex flex-col min-h-0 bg-neutral-900 overflow-hidden" id="grid-navigator">
        ...
        {bibleTab === 'browse' ? (
          <BibleGrid
            ...
            versesInChapterCount={versesInChapterCount}
            handleTriggerProjectVerse={handleTriggerProjectVerse}
          />
        ) : (
          <BibleSearchPanel
            searchQuery={searchQuery}
            setSearchQuery={setSearchQuery}
            parsedLookupResult={parsedLookupResult}
            ...
            setActiveChapter={setActiveChapter}
            setActiveVerse={setActiveVerse}
            getFormattedProjectionAndLabel={getFormattedProjectionAndLabel}
            onProjectText={onProjectText}
      ...
      <BibleConfigModal
        ...
        setReferenceTranslation={setReferenceTranslation}
        loadedBibles={loadedBibles}
        setLoadedBibles={setLoadedBibles}
        ...
        setBibleLayout={setBibleLayout}
        descStyle={descStyle}
        setDescStyle={setDescStyle}
        descSeparator={descSeparator}
        setDescSeparator={setDescSeparator}
        descPosition={descPosition}
        setDescPosition={setDescPosition}
        descShowVersion={descShowVersion}
        setDescShowVersion={setDescShowVersion}
        descLineHeight={descLineHeight}
        setDescLineHeight={setDescLineHeight}
        descAlignment={descAlignment}
        setDescAlignment={setDescAlignment}
